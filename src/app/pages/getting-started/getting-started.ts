...
@Component({
  ...
  template: `
    <div class="doc-page">
      ...
      <p>
        ...
        。Release 通常包含多个 ABI 分包（<code>armeabi-v7a</code>、<code>arm64-v8a</code>、<code>x86</code>、<code>x86_64</code>）以及一个 universal 包。
        <b>普通电视用户优先尝试 <code>armeabi-v7a</code></b>，装上跑不起来再换 <code>arm64-v8a</code> 或 <code>universal</code>。
      </p>
      ...
      <h2>4. 配置订阅源</h2>
      <p>订阅源是 <b>电视直播</b> 的核心数据。<b>实际使用建议自行准备 m3u 订阅</b>。添加订阅源有三种方式：</p>
      <ol>
  ...
  -d '&#123;"name":"我的源","type":"url","url":"https://example.com/iptv.m3u"&#125;'</code></pre>
      ...
      <p>详细的订阅源格式与示例，见 <a [routerLink]="'/sources'">订阅源</a> 章节。</p>

      <h2>5. 配置 EPG（可选）</h2>
      <p>
        默认 EPG 已经能用。如需更换或追加来源，进入 <b>设置 → 节目单 → 自定义节目单</b>，支持 XML / XML_GZ / DIYP / LOVETV / CHUNKED_XML 多种格式。
        详见 <a [routerLink]="'/epg'">EPG 节目单</a>。
      </p>

      <h2>6. 远程配置面板（推荐）</h2>
      <p>
        电视遥控器改大量设置不方便。<b>电视直播</b>启动后会监听局域网 <code>10591</code> 端口，
      ...
      <doc-callout kind="tip" title="建议的下一步" icon="lightbulb">
        先按 <a [routerLink]="'/controls'">遥控器与触屏</a> 章节熟悉所有按键，再按
        <a [routerLink]="'/settings'">设置项总览</a> 把每个设置项过一遍，能避开 80% 的"为什么没生效"。
      </doc-callout>
    </div>
  `,
})
